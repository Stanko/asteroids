...
export type UiBindings = {
  onParamPatch: (patch: Partial<AppParams>, options?: { regenerate?: boolean }) => void;
  onRandomize: () => void;
  onGenerate: () => void;
};
...
export function createUi(params: AppParams, bindings: UiBindings): AppUi {
  ...
  frameFolder.add(params, 'exportSizePx', 16, 256, 1).name('exportSizePx').onChange((value: number) => {
    bindings.onParamPatch({ exportSizePx: value });
  });
  frameFolder.add(params, 'rotationSteps', 1, 256, 1).name('rotationSteps').onChange((value: number) => {
    bindings.onParamPatch({ rotationSteps: value });
  });

  const shapeFolder = gui.addFolder('Asteroid');
  shapeFolder.add(params, 'seed').name('seed').onFinishChange((value: string) => {
    bindings.onParamPatch({ seed: value }, { regenerate: true });
  });
  shapeFolder.add(params, 'distortion', 0, 5, 0.001).name('distortion').onChange((value: number) => {
    bindings.onParamPatch({ distortion: value }, { regenerate: true });
  });
  shapeFolder.add(params, 'size', 0.4, 1.2, 0.001).name('size').onChange((value: number) => {
    bindings.onParamPatch({ size: value }, { regenerate: true });
  });

  const shadeFolder = gui.addFolder('Shading');
  shadeFolder.add(params, 'toonSteps', 2, 8, 1).name('toonSteps').onChange((value: number) => {
    bindings.onParamPatch({ toonSteps: value });
  });
  shadeFolder.add(params, 'lightIntensity', 0, 3, 0.001).name('lightIntensity').onChange((value: number) => {
    bindings.onParamPatch({ lightIntensity: value });
  });
  shadeFolder.add(params, 'ambientIntensity', 0, 1, 0.001).name('ambientIntensity').onChange((value: number) => {
    bindings.onParamPatch({ ambientIntensity: value });
  });
  shadeFolder.add(params, 'flatShading').name('flatShading').onChange((value: boolean) => {
  ...
  paletteFolder.addColor(paletteProxy, 'p0').name('palette[0]').onChange((value: string) => updatePalette(0, value));
  paletteFolder.addColor(paletteProxy, 'p1').name('palette[1]').onChange((value: string) => updatePalette(1, value));
  paletteFolder.addColor(paletteProxy, 'p2').name('palette[2]').onChange((value: string) => updatePalette(2, value));
  paletteFolder.addColor(paletteProxy, 'p3').name('palette[3]').onChange((value: string) => updatePalette(3, value));
  paletteFolder.addColor(paletteProxy, 'outline').name('outlineColor').onChange((value: string) => {
    bindings.onParamPatch({ outlineColor: value as AppParams['outlineColor'] });
  });
